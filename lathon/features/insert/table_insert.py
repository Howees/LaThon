...
class BaseTableDialog(BaseModal):
    """
    Motor visual responsável pela construção de tabelas interativas (Grid Dinâmico).
    Inclui lógicas complexas de redimensionamento manual e renderização de matrizes para interface (Tkinter)
    que posteriormente serão transcritas para código no insert_manager.py.
    """

    def __init__(self, master_app, title, default_r, default_c, default_w, default_h, min_w, min_h,
                 default_label="tab:"):
        ...
        # ==========================================
        # TOOLBAR DE ESTILIZAÇÃO E ALINHAMENTO
        # ==========================================
        ...
        ctk.CTkLabel(self.toolbar_frame, text="Alinhamento:", font=Fonts.UI_BOLD).pack(side="left", padx=(10, 5),
                                                                                       pady=8)

        self.valign_combo = ctk.CTkOptionMenu(self.toolbar_frame, values=["Topo", "Meio", "Base"], width=90,
                                              fg_color=Colors.BG_MAIN, button_color=Colors.THON)
        self.valign_combo.set("Meio")
        self.valign_combo.pack(side="left", padx=(0, 5))

        self.halign_combo = ctk.CTkOptionMenu(self.toolbar_frame, values=["Esquerda", "Centro", "Direita"], width=90,
                                              fg_color=Colors.BG_MAIN, button_color=Colors.THON)
        ...
        self.border_combo = ctk.CTkOptionMenu(self.toolbar_frame, values=estilos, width=180, fg_color=Colors.BG_MAIN,
                                              button_color=Colors.THON)
        self.border_combo.set("Grade")
        self.border_combo.pack(side="left", padx=(0, 15))

        # ==========================================
        # RODAPÉ (LEGENDA E LABEL)
        # ==========================================
        ...
        self.grid_container = ctk.CTkFrame(self.border_frame, fg_color=Colors.BG_SIDEBAR, border_width=1,
    ...
    # Stub methods to be implemented by child classes
    ...
    def _build_counter_and_size(self, parent, text, initial_val, callback_count, callback_direct, text_size,
                                callback_size):
        """Constrói os widgets padrão de incremento (+/-) com validação visual."""
        f = ctk.CTkFrame(parent, fg_color="transparent")
        f.pack(side="left")

        ctk.CTkLabel(f, text=text, font=Fonts.UI).pack(side="left", padx=(0, 5))
        btn_style = {"width": 24, "height": 24, "font": Fonts.UI_BOLD, "fg_color": Colors.BG_MAIN,
                     "text_color": Colors.TEXT_NORMAL, "hover_color": Colors.BTN_HOVER, "border_width": 1,
                     "border_color": Colors.BORDER}
    ...
    def _apply_r_direct(self, event=None):
        try:
            val = int(self.entry_rows.get())
        except ValueError:
            self.entry_rows.delete(0, 'end')
            self.entry_rows.insert(0, str(self.current_r))
            return
    ...
    def _apply_c_direct(self, event=None):
        try:
            val = int(self.entry_cols.get())
        except ValueError:
            self.entry_cols.delete(0, 'end')
            self.entry_cols.insert(0, str(self.current_c))
            return
        delta = val - self.current_c
        if delta != 0: self._change_cols(delta)

    def _update_size_entries(self):
        """Avalia as células selecionadas e exibe a largura e altura corretas no formulário superior."""
        ...
        if len(h_set) == 1:
            self.entry_h.insert(0, str(list(h_set)[0]))
        elif len(h_set) > 1:
            self.entry_h.insert(0, "---")

        self.entry_w.delete(0, 'end')
        if len(w_set) == 1:
            self.entry_w.insert(0, str(list(w_set)[0]))
        elif len(w_set) > 1:
            self.entry_w.insert(0, "---")
    ...
    def _apply_h(self, event=None):
        """Redimensiona as alturas das células, validando limite físico de espaço na janela."""
        val_str = self.entry_h.get()
        if val_str == "---" or not val_str: return

        try:
            val = max(self.min_h, int(val_str))
        except ValueError:
            self._update_size_entries(); return
    ...
    def _apply_w(self, event=None):
        """Redimensiona as larguras das células garantindo que não escapem do canvas (avail space)."""
        val_str = self.entry_w.get()
        if val_str == "---" or not val_str: return

        try:
            val = max(self.min_w, int(val_str))
        except ValueError:
            self._update_size_entries(); return
    ...
    def _refresh_indicators(self):
        """Repinta os marcadores laranjas de seleção externa da tabela."""
        ...
        for c, ind in enumerate(self.col_indicators):
            ind.configure(
                fg_color=Colors.THON if (c in self.selected_cols and not self.is_select_all) else Colors.BTN_HOVER)
        for r, ind in enumerate(self.row_indicators):
            ind.configure(
                fg_color=Colors.THON if (r in self.selected_rows and not self.is_select_all) else Colors.BTN_HOVER)
    ...
    def _draw_grid(self):
        """Mecânica core: Roda pelas matrizes de dimensão e constrói a grade frame-a-frame no Tkinter."""
        saved_data = []
    ...
    def _move_focus(self, r, c, direction):
        """Permite navegar fluido entre as células da tabela apertando 'Tab' ou 'Shift+Tab'."""
        nc = c + direction
        nr = r
        ...
        if 0 <= nr < self.current_r and 0 <= nc < self.current_c:
            try:
                ...
                if isinstance(cell, ctk.CTkTextbox):
                    cell.focus_set()
                elif hasattr(cell, 'entry'):
...
class TableInputDialog(BaseTableDialog):
    """Construtor da Tabela Simples (Apenas conteúdo textual via CTkTextbox)."""

    def __init__(self, master_app):
        super().__init__(master_app, "Inserir Tabela Visual", default_r=4, default_c=8, default_w=60, default_h=30,
                         min_w=20, min_h=20, default_label="tab:")

    def _create_cell(self, parent, r, c, w, h, initial_data):
        entry = ctk.CTkTextbox(parent, width=w, height=h, corner_radius=0, fg_color=Colors.BG_PANEL,
                               text_color=Colors.TEXT_NORMAL, border_width=0, wrap="word", font=Fonts.UI)
...
class ImageTableInputDialog(BaseTableDialog):
    """Construtor Misto de Tabelas (Permite alternar a célula entre inserção de Imagem Local ou Texto livre)."""

    def __init__(self, master_app):
        super().__init__(master_app, "Tabela de Figuras", default_r=2, default_c=2, default_w=180, default_h=150,
                         min_w=20, min_h=20, default_label="tab:fig_")
    ...
    def _create_cell(self, parent, r, c, w, h, initial_data):
        ...
        entry = ctk.CTkTextbox(cell_frame, fg_color="transparent", border_width=0, corner_radius=0,
                               text_color=Colors.TEXT_NORMAL, font=Fonts.UI, wrap="word")

        btn_add = ctk.CTkButton(cell_frame, text="", corner_radius=4, fg_color="transparent",
                                hover_color=Colors.BTN_HOVER)
        img_display = ctk.CTkLabel(cell_frame, text="", text_color=Colors.TEXT_NORMAL)
        img_tooltip = ToolTip(img_display, "")
        btn_trash = ctk.CTkButton(cell_frame, text="", corner_radius=4, fg_color="transparent",
        ...
        def update_ui():
            """Controla a troca visual da célula (Mostra o Editor de texto ou Cobre tudo exibindo a miniatura da Imagem)."""
            ...
            # Utiliza o gerenciador nativo cacheado do design.py (preparado para executável)
            try:
                icn_add = Icons.get_ctk_image("image.png", size=icn_sz)
                icn_trash = Icons.get_ctk_image("trash.png", size=icn_sz)
            except Exception:
            ...
            if cell_frame.mode == "text":
                ...
                img_tooltip.text = ""
                # Só exibe o atalho de "+" para adicionar imagem se o campo texto estiver vazio
        ...
        def select_image():
            """Abre o seletor de arquivos e converte a célula para exibir a nova imagem setada."""
            if not cell_frame.winfo_exists(): return
        ...
        def clear_image():
            """Reverte uma célula de imagem de volta para uma célula livre de edição de texto."""
            if not cell_frame.winfo_exists(): return
        ...
        entry.bind("<FocusOut>", lambda e: btn_add.place(relx=0.98, rely=0.05, anchor="ne") if not entry.get("1.0",
                                                                                                             "end-1c").strip() else None)
        entry.bind("<Tab>", lambda e=None: handle_key("next", e))
        entry.bind("<Shift-Tab>", lambda e=None: handle_key("prev", e))
