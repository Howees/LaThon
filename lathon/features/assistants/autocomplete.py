...
# Snippets de blocos estruturais comuns
SNIPPETS = {
...
# Comandos de uso frequente para autocomplete rápido
LATEX_COMMANDS = [
...
class AutocompleteHandler:
    """Gerencia a captura de teclas, análise de contexto e exibição do popup de sugestões."""
    ...
    def __init__(self, app, editor):
        ...
        # Delegação de atalhos exclusivamente para o escopo do editor
    ...
    def _on_focus_out(self, event=None):
        """Garante que o popup seja fechado se o usuário clicar fora do editor."""
    ...
    def _get_project_data(self):
        """
        Faz uma varredura no diretório do projeto em busca de
        arquivos e referências para alimentar as sugestões de \\ref e \\cite.
        """
        ...
        for p in self.app.project_dir.rglob('*'):
            if p.is_file():
                ...
                if p.suffix.lower() in FILE_EXTENSIONS:
                    files.append(p.relative_to(self.app.project_dir).as_posix())
                if p.suffix.lower() in BIB_EXTENSIONS:
                    bib_files.append(p.stem)
        ...
        # Parseamento específico de chaves BibTeX
    ...
    def _on_key_release(self, event):
        """
        Analisa a string imediatamente antes do cursor a cada tecla digitada
        para decidir qual contexto de autocomplete exibir.
        """
        ...
        # Isola a palavra que está sendo digitada atualmente
        ...
        # 1. Contexto de Argumento (dentro de chaves {} de comandos específicos)
        ...
        # 2. Contexto de Snippets Dinâmicos (ex: digitando 'fig' -> sugere estrutura figure inteira)
        ...
        # 3. Contexto de Comandos Nativos do LaTeX (iniciados com contrabarra)
    ...
    def _show_suggestions_popup(self, suggestions):
        """Constrói ou atualiza a janela flutuante sob o cursor do usuário."""
        ...
        if not self.autocomplete_window:
            ...
            self.suggestions_listbox = tk.Listbox(
                self.autocomplete_window, height=5, font=Fonts.MONO,
        ...
        # Cálculo dinâmico para posicionar o popup imediatamente abaixo das coordenadas do cursor
        try:
    ...
    def _select_suggestion(self, event=None):
        """Injeta a sugestão selecionada no texto, substituindo o fragmento digitado."""
        if not self.autocomplete_window or self.autocomplete_window.state() == "withdrawn":
            return
        ...
        if self.suggestion_type == 'snippet':
            ...
            # Posiciona o cursor automaticamente dentro das chaves, se existirem
            if "{}" in text:
                self.editor.mark_set("insert", "insert -1 chars")

        self._hide_suggestions()
        return "break"

    def _move_selection(self, event):
        """Permite navegar na lista de sugestões usando as setas (Cima/Baixo) do teclado."""
        if not self.autocomplete_window or self.autocomplete_window.state() == "withdrawn":
            return
