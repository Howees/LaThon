...
class MarkersListDialog(BaseModal):
    ...
    def __init__(self, app):
        ...
        ctk.CTkLabel(self.border_frame, text="Anotações no Documento", font=("Segoe UI", 14, "bold")).pack(anchor="w",
                                                                                                           padx=20,
                                                                                                           pady=(20, 5))

        self.scroll = ctk.CTkScrollableFrame(self.border_frame, fg_color=("gray90", "#1e1e1e"))
    ...
    def _populate_markers(self):
        ...
        if not comments:
            ctk.CTkLabel(self.scroll, text="Nenhuma marcação no documento atual.", text_color="gray50").pack(pady=20)
            return
        ...
        for tag, comment in comments.items():
            ...
            ctk.CTkLabel(f, text=text_disp, anchor="w", justify="left", font=("Segoe UI", 11)).pack(side="left",
                                                                                                    fill="x",
                                                                                                    expand=True)

            ctk.CTkButton(f, text="Ir", width=40, height=24, font=("Segoe UI", 10),
